#!/usr/bin/env python3
# 实现双机械臂的导纳控制，基于空间误差和动力学计算。
# 实现夹爪的初始张开，并在控制循环中根据外部作用力调整末端位置和姿态。
import threading
...
class BimanualAdmittance:
    def __init__(self, model_path="xml/surface_force_control.xml"):
        ...
        # ==== 初始姿态（7关节手臂 + 2关节手爪） ====
        q_init_arm = np.array([0, -0.785, 0, -2.356, 0, 3.14, 0.785])
        q_init_finger = np.array([0.02, 0.02])  # 手爪初始张开 2cm
        self.finger_actuator_left = self.model.actuator("panda_left_actuator_finger").id
        self.finger_actuator_right = self.model.actuator("panda_right_actuator_finger").id

        # 左手
        for i, adr in enumerate(self.dof_left):
            self.data.qpos[adr] = q_init_arm[i]
        self.data.qpos[self.model.joint("panda_left_finger_joint1").dofadr[0]] = q_init_finger[0]
        self.data.qpos[self.model.joint("panda_left_finger_joint2").dofadr[0]] = q_init_finger[1]

        # 右手
        for i, adr in enumerate(self.dof_right):
            self.data.qpos[adr] = q_init_arm[i]
        self.data.qpos[self.model.joint("panda_right_finger_joint1").dofadr[0]] = q_init_finger[0]
        self.data.qpos[self.model.joint("panda_right_finger_joint2").dofadr[0]] = q_init_finger[1]

        mujoco.mj_forward(self.model, self.data)

        pL = self.data.body("panda_left_hand").xpos.copy()
        pR = self.data.body("panda_right_hand").xpos.copy()
    ...
    def start_gui(self):
        def gui_thread():
            root = tk.Tk()
            root.title("Finger Force Control")

            # 左手滑条
            tk.Label(root, text="Left Finger Force").pack()
            left_slider = tk.Scale(root, from_=-35, to=35, orient=tk.HORIZONTAL, length=300,
                                   command=lambda v: setattr(self, 'left_force', float(v)))
            left_slider.set(0)
            left_slider.pack()

            # 右手滑条
            tk.Label(root, text="Right Finger Force").pack()
            right_slider = tk.Scale(root, from_=-35, to=35, orient=tk.HORIZONTAL, length=300,
                                    command=lambda v: setattr(self, 'right_force', float(v)))
            right_slider.set(0)
            right_slider.pack()
    ...
    def control_step(self, dt):
        ...
        # ===== LEFT HAND =====
        if norm_L > 1e-3:
            acc = np.linalg.inv(self.M_adm) @ (W_L - self.D_adm @ self.adm_vel_L - self.K_adm @ self.adm_pos_L)
        ...
        # ===== RIGHT HAND =====
        if norm_R > 1e-3:
            acc = np.linalg.inv(self.M_adm) @ (W_R - self.D_adm @ self.adm_vel_R - self.K_adm @ self.adm_pos_R)
        ...
        # LEFT finger
        f_ll = self.data.sensor("panda_left_left_finger_force").data.copy()
        f_lr = self.data.sensor("panda_left_right_finger_force").data.copy()

        # RIGHT finger
        f_rl = self.data.sensor("panda_right_left_finger_force").data.copy()
        f_rr = self.data.sensor("panda_right_right_finger_force").data.copy()
        tau_full[7] = self.left_force
        tau_full[15] = self.right_force
    ...
    def run(self):
        with mujoco.viewer.launch_passive(self.model, self.data) as viewer:
            while viewer.is_running():
                ...
                # 写入 actuator
                self.data.ctrl[self.finger_actuator_left] = self.left_force
                self.data.ctrl[self.finger_actuator_right] = self.right_force
