...
class VariableJointImpedanceAdmittanceMouse:
    """
    Hybrid controller:
    1) Admittance layer: external wrench -> adm_vel / adm_pos
    2) Differential IK layer: admittance velocity -> moving joint reference q_ref
    3) Variable joint impedance: use dq and wrench to solve joint kp / kd online

    Admittance axis order:
    [x, y, z, rx, ry, rz]

    Spatial task / Jacobian axis order:
    [rx, ry, rz, x, y, z]
    """
    ...
    def set_direction_mask(self, x=1.0, y=1.0, z=1.0, rx=1.0, ry=1.0, rz=1.0):
    ...
    def get_state(self):
        return {
            ...
            "adm_vel": self.adm_vel.copy(),
            "motion_mask_axes": ("x", "y", "z", "rx", "ry", "rz"),
            "motion_mask": self.motion_mask.copy(),
        }

    def set_debug_print(self, enabled=True, interval=0.2):
        self.enable_debug_print = bool(enabled)
        self.print_interval = float(interval)
    ...
    def update_joint_reference(self, q, dt):
        # admittance velocity [x, y, z, rx, ry, rz] -> task velocity [rx, ry, rz, x, y, z]
        ...
        qref_dot = J_pinv @ task_twist_ref
        qref_dot = np.clip(qref_dot, -self.qref_rate_limit, self.qref_rate_limit)
    ...
    def maybe_print_debug(self, wrench_task):
        if not self.enable_debug_print:
            return
        if self.data.time - self._last_print_time < self.print_interval:
            return

        self._last_print_time = self.data.time
        q_ref_str = np.array2string(self.q_ref, precision=3, suppress_small=True)
        kp_str = np.array2string(self.kp_diag, precision=1, suppress_small=True)
        kd_str = np.array2string(self.kd_diag, precision=1, suppress_small=True)
        wrench_str = np.array2string(wrench_task, precision=3, suppress_small=True)
        adm_pos_str = np.array2string(self.adm_pos, precision=4, suppress_small=True)
        print(
            f"[t={self.data.time:7.3f}] "
            f"wrench={wrench_str} "
            f"adm_pos={adm_pos_str} "
            f"q_ref={q_ref_str} "
            f"kp={kp_str} "
            f"kd={kd_str}"
        )
    ...
    def compute_control(self, dt):
        ...
        self.maybe_print_debug(wrench_task)
        return tau, dq, wrench_task, tau_ext

    def control_step(self, dt):
        tau, _, _, _ = self.compute_control(dt)
...
if __name__ == "__main__":
    sim = VariableJointImpedanceAdmittanceMouse(
        model_path="scene.xml",
        urdf_path="fr3v2.urdf",
        ee_body_name="fr3v2_link7",
        ee_frame_name="fr3v2_link8",
