#!/usr/bin/env python3
"""
机器人动力学量计算器 - 使用 Pinocchio（修正版）

修复点：
1. 不再使用“最后 7 个驱动关节”这种危险逻辑。
2. 显式按关节名绑定 panda_joint1 ~ panda_joint7，避免把 finger joints 混进来。
...
class DynamicsCalculatorPinocchio:
    ...
    def __init__(
        ...
        try:
            if root_joint_type is None:
                ...
                self.model = pin.buildModelFromUrdf(urdf_path, root_joint_type)
        ...
        print(f"  Pinocchio 模型: nq={self.model.nq}, nv={self.model.nv}")
        print(f"  固定基座: {self.has_fixed_base}")
        print(f"  驱动关节数量: {self.n_joints}")
        print("  驱动关节映射:")
        for name, v_idx in zip(self.actuated_joint_names, self.actuated_joint_indices):
            print(f"    {name:<20} -> idx_v={v_idx}")
        print(f"  末端执行器框架: {self.end_effector_name} (ID: {self.end_effector_id})")
        print("✓ Pinocchio 动力学计算器初始化完成（修正版）")

    def _resolve_actuated_joint_indices(self, joint_names: Sequence[str]) -> list:
        """按 joint 名称显式解析 idx_v，避免 finger joints 混入。"""
        resolved = []
        available_joint_names = [self.model.names[self.model.joints[i].id()] if hasattr(self.model.joints[i], 'id') else None for i in range(self.model.njoints)]
        # 上面 available_joint_names 对某些 pin 版本不稳定，下面用 names 列表兜底打印更靠谱
        available_joint_names = [self.model.names[self.model.joints[i].idx_q:self.model.joints[i].idx_q] if False else None for i in range(0)]
        del available_joint_names

        all_names = [self.model.names[self.model.joints[i].idx_q:self.model.joints[i].idx_q] for i in range(0)]
        del all_names

        model_joint_names = [self.model.names[self.model.joints[i].id()] for i in range(0)] if False else None
        del model_joint_names

        # Pinocchio Python 常用接口：existJointName / getJointId
        for name in joint_names:
            if not self.model.existJointName(name):
                candidate_names = [self.model.names[self.model.joints[i].idx_q:self.model.joints[i].idx_q] for i in range(0)] if False else []
                raise ValueError(
                    f"Pinocchio 模型中找不到关节 '{name}'。"
                )
    ...
    def _resolve_end_effector_frame(self, requested_name: str):
        ...
        # 常见 panda 末端候选，优先 hand
        fallback_candidates = [
            "panda_hand",
            "panda_hand_tcp",
            "panda_link8",
            "panda_link8_virtual_joint",
            "panda_hand_tcp_virtual_joint",
        ]
